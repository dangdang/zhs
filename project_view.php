...
$sql=sprintf("SELECT * , sum(csa_tb_manhour) as summ1 FROM tk_task_byday
							inner join tk_task_tpye on tk_task_byday.csa_tb_backup4=tk_task_tpye.id
								WHERE csa_tb_backup3 = %s GROUP BY csa_tb_backup4 ORDER BY summ1 DESC", GetSQLValueString($colname_DetailRS1, "text"));
...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
    <tr>
      ...
      <td valign="top">
         <div style="overflow:auto; " id="main_right"><!-- right main -->
          ...
          <tr>
              ...
              <table width="100%" border="0" cellspacing="0" cellpadding="5"  class="info_task_bg">
                  <tr>
                  <!--项目负责人-->
                    <td width="12%" class="info_task_title"><?php echo $multilingual_project_touser; ?></td>
                    <td width="40%">
                         <?php $userArr=explode(',',$row_DetailRS1['project_manager']); $num=count($userArr);
        ...
        <!--项目院长-->
		<td  width="12%" class="info_task_title">项目院长</td>
		<td width="40%">
		    <?php $userArr=explode(',',$row_DetailRS1['project_dean']); $num=count($userArr);
            for($index=0;$index<$num;$index++)
            {?><a href="user_view.php?recordID=<?php echo trim($userArr[$index]); ?>"><?php echo $userNameArray[trim($userArr[$index])];if ($num>1 and $index<($num-1)){echo ",";}; ?></a>&nbsp;
            <?php } ?>
		</td>
      ...
      <tr>
		<!--项目主管总工-->
		<td class="info_task_title">项目主管总工</td>
		<td>
                        <?php $userArr=explode(',',$row_DetailRS1['project_chief_engineer']); $num=count($userArr);
            for($index=0;$index<$num;$index++)
            {?><a href="user_view.php?recordID=<?php echo trim($userArr[$index]); ?>"><?php echo $userNameArray[trim($userArr[$index])];if ($num>1 and $index<($num-1)){echo ",";}; ?></a>&nbsp;
            <?php } ?>
		</td>
		...
		<!--项目主管所长-->
		<td class="info_task_title">项目主管所长</td>
		<td>
                        <?php $userArr=explode(',',$row_DetailRS1['project_Director']); $num=count($userArr);
            for($index=0;$index<$num;$index++)
            {?><a href="user_view.php?recordID=<?php echo trim($userArr[$index]); ?>"><?php echo $userNameArray[trim($userArr[$index])];if ($num>1 and $index<($num-1)){echo ",";}; ?></a>&nbsp;
            <?php } ?>
		</td>
	  ...
	  <tr>
		<!--项目主管主任工程师-->
		<td class="info_task_title">项目主管主任工程师</td>
		<td>
                            <?php $userArr=explode(',',$row_DetailRS1['project_Division_engineer']); $num=count($userArr);
                for($index=0;$index<$num;$index++)
                {?><a href="user_view.php?recordID=<?php echo trim($userArr[$index]); ?>"><?php echo $userNameArray[trim($userArr[$index])];if ($num>1 and $index<($num-1)){echo ",";}; ?></a>&nbsp;
                <?php } ?>
		</td>
		...
		<td>
                        <?php $userArr=explode(',',$row_DetailRS1['project_member']); $num=count($userArr);
            for($index=0;$index<$num;$index++)
            {?><a href="user_view.php?recordID=<?php echo trim($userArr[$index]); ?>"><?php echo $userNameArray[trim($userArr[$index])];if ($num>1 and $index<($num-1)){echo ",";}; ?></a>&nbsp;
            <?php } ?>
		</td>
          ...
          <tr>
              ...
              <table width="100%" border="0" cellspacing="0" cellpadding="5"  class="info_task_bg">
                  <tr>
                    <td width="12%" class="info_task_title">项目类型</td>
                    <td  width="40%"><?php echo $row_DetailRS1['project_type_name']; ?>&nbsp; </td>
                    <td  width="12%" class="info_task_title">项目重要度</td>
                    <td><?php echo $row_DetailRS1['project_importance']; ?>&nbsp;</td>
                  </tr>
                  <tr>
...
//下发任务、新建文件夹、新建文档等功能
    ...
    <!---->
          ...
          <tr>
            <td>
<!--                <div id="chart"></div>		原flash版的图表-->
                         <!-- 为ECharts准备一个具备大小（宽高）的Dom -->
                <div id="main_projecthour" style="width:600px;height:230px"></div>
                <!-- ECharts单文件引入 -->
                <script src="chart/echarts/echarts.js"></script>
                <script type="text/javascript">
                    // 路径配置
                    require.config({
                        paths: {
            ...
            //                'echarts',
                        ...
                        function (ec) {
                            ...
                            var myChart = ec.init(document.getElementById('main_projecthour'));
            ...
            //                            subtext: '纯属虚构',
                                    ...
                                    legend: {
                                        orient : 'vertical',
                                        x : 'left',
                                       data:<?php echo json_encode($dis_name)?>
                                             },
                                    toolbox: {
                                        show : true,
                                        feature : {
