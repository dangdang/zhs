...
function GetSQLValueString($theValue, $theType, $theDefinedValue = "", $theNotDefinedValue = "") 
{
  ...
  }//在magic_quotes_gpc=On的情况下，如果输入的数据有单引号（’）、双引号（”）、反斜线（）与 NUL（NULL 字符）等字符都会被加上反斜线，以确保这些数据不会引起程序，特别是数据库语句因为特殊字符引起的污染而出现致命的错误
   //如果magic_quotes_gpc=On,则用stripslashes去掉多余的"/",
...
return $row_Recordset_item['tk_item_value'];//从tk_item表中返回一个值，tk_item表主要是后台设置各种参数
}
...
//get user
function get_user_select($user_role_order=0,$user_rank=0) {
global $tankdb;
...
if($user_rank>0){
	$query_user .=" and tk_user.tk_user_rank =".$user_rank;
}else{
	$query_user .=" and tk_user.tk_user_rank <> '0'";
}
$query_user .=" ORDER BY CONVERT(tk_user.tk_display_name USING gbk )";
...
$user_arr[$row_user['uid']]['pinyin'] =$row_user['tk_pinyin']; //modified by zhl,20150306
