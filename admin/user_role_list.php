...
if ((isset ( $_POST ["MM_update"] )) && ($_POST ["MM_update"] == "form1")) {
	$updateSQL = sprintf ( "UPDATE tk_user_role SET tk_role_name=%s,tk_role_order=%s WHERE id=%s", GetSQLValueString ( $_POST ['role_name'], "text" ),GetSQLValueString ( $_POST ['role_order'], "int" ),GetSQLValueString ( $_POST ['id'], "int" ) );
...
if (isset ( $_GET ['editID'] )) {
	...
	$qu = sprintf ( "SELECT * FROM tk_user_role WHERE id = %s", GetSQLValueString ( $_GET ['editID'], "int" ) );
	echo $qu;
...
if ((isset ( $_GET ['delID'] )) && ($_GET ['delID'] != "") && ($_SESSION ['MM_Username'] != $multilingual_dd_user_readonly)) {
	$deleteSQL = sprintf ( "DELETE FROM tk_user_role WHERE id=%s", GetSQLValueString ( $_GET ['delID'], "int" ) );
	$Result1 = mysql_query ( $deleteSQL, $tankdb ) or die ( mysql_error () );
}

// 生成写入数据库的sql语句
if ((isset ( $_POST ["MM_insert"] )) && ($_POST ["MM_insert"] == "form1")) {
	$insertSQL = sprintf ( "INSERT INTO tk_user_role (tk_role_name,tk_role_order) VALUES (%s,%s)", GetSQLValueString ( $_POST ['role_name'], "text" ),GetSQLValueString ( $_POST ['role_order'], "int" ));
...
J.check.rules = [
	{name: 'role_name', mid: 'role_name_msg', requir: true, type: '',  warn: '<?php echo $multilingual_default_required4; ?>'},
	{name: 'role_order', mid: 'role_order_msg', type: 'rang', min: -1, warn: '<?php echo $multilingual_default_required5; ?>' }
...
<?php do { ?>
    <tr>
							<td width="50%"><?php echo $row_Recordset1['tk_role_name']; ?></td>
							<td width="20%"><?php echo $row_Recordset1['tk_role_order']; ?></td>
							<td width="15%"><a
