...
if ((isset ( $_POST ["log_insert"] )) && ($_POST ["log_insert"] == "form1")) {
	$insertSQL = sprintf ( "INSERT INTO tk_task_byday (csa_tb_text, csa_tb_year, csa_tb_status, csa_tb_manhour, csa_tb_backup1, csa_tb_backup2, csa_tb_backup3, csa_tb_backup4) VALUES ($csa_tb_text %s, %s, %s, %s, %s, %s, %s)", GetSQLValueString ( $_POST ['csa_tb_year'], "text" ), GetSQLValueString ( $_POST ['csa_tb_status'], "text" ), GetSQLValueString ( $_POST ['csa_tb_manhour'], "text" ), GetSQLValueString ( $_POST ['csa_tb_backup1'], "text" ), GetSQLValueString ( $_POST ['csa_tb_backup2'], "text" ), GetSQLValueString ( $_POST ['csa_tb_backup3'], "text" ), GetSQLValueString ( $_POST ['csa_tb_backup4'], "text" ) );
...
if ((isset ( $_POST ["task_update"] )) && ($_POST ["task_update"] == "form1")) {
	$updateSQL = sprintf ( "UPDATE tk_task SET csa_remark2=%s, csa_remark3=%s, csa_last_user=%s WHERE TID=%s", GetSQLValueString ( $_POST ['csa_tb_status'], "text" ), GetSQLValueString ( $_POST ['csa_tb_time'], "text" ), GetSQLValueString ( $nowuser, "text" ), GetSQLValueString ( $taskid, "int" ) );
	mysql_select_db ( $database_tankdb, $tankdb );
...
<link href="skin/themes/base/lhgdialog.css" rel="stylesheet"
	type="text/css" />
<link href="skin/themes/base/jquery-ui.min.css" rel="stylesheet"
	type="text/css" />
<title>log</title>
...
<?php echo $logyear[0]; ?>-<?php echo $logmonth[0]; ?>-<?php echo $logmonth[1]; ?> <?php echo $multilingual_default_task_status; ?></span>
		...
		<div class="modal-footer"
			style="margin-top: 0px; padding: 10px 10px 10px;">
			<button type="button" id="btn1" class="btn btn-default btn-sm"
				onclick="over()"><?php echo $multilingual_global_action_cancel; ?></button>
			<button name="cont" type="button" class="btn btn-primary btn-sm"
				data-loading-text="<?php echo $multilingual_global_wait; ?>"
				onClick="submitform()"><?php echo $multilingual_global_action_save; ?></button>

			<input type="submit" id="btn5"
				value="<?php echo $multilingual_global_action_save; ?>"
				style="display: none" /> <input type="hidden" name="csa_tb_year"
				id="csa_tb_year" value="<?php echo $logdate; ?>" /> <input
				type="hidden" name="csa_tb_backup1" id="csa_tb_backup1"
				value="<?php echo $taskid; ?>" /> <input type="hidden"
				name="csa_tb_backup2" id="csa_tb_backup2"
				value="<?php echo $userid; ?>" /> <input type="hidden"
				name="csa_tb_backup3" id="csa_tb_backup3"
				value="<?php echo $projectid; ?>" /> <input type="hidden"
				name="csa_tb_backup4" id="csa_tb_backup4"
				value="<?php echo $tasktype; ?>" /> <input type="hidden"
				name="csa_tb_time" id="csa_tb_time"
				value="<?php echo date("Y-m-d H:i:s"); ?>" /> <input type="hidden"
				name="log_insert" value="form1" /><input type="hidden"
				name="task_update" value="form1" />
		</div>
