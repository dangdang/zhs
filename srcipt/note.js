$(document).ready(function(){
	...
	$('.color').live('click',function(){
		$('#fancy_ajax .note').removeClass('yellow green blue pink black purple gray brown syan red').addClass($(this).attr('class').replace('color',''));
	});
	...
	$('#note-submit').live('click',function(e){
		...
		$(this).replaceWith('<img src="/public/images/fancybox/ajax_load.gif" style="margin:30px auto;display:block" />');
		...
		$.post('/note/post',data,function(msg){
	...
	$('#note_delete').live('click',function(){
		var x=$(this).parent();
		$.get('/note/update_status',{
			  s		: 1,
			  id	: parseInt(x.find('span.data').html())
		},function(d){
			x.fadeOut();
		});
	});
