...
$sql=sprintf("SELECT * , sum(csa_tb_manhour) as summ1 FROM tk_task_byday
							inner join tk_task_tpye on tk_task_byday.csa_tb_backup4=tk_task_tpye.id
								WHERE csa_tb_backup2 = %s GROUP BY csa_tb_backup4 ORDER BY summ1 DESC", GetSQLValueString($colname_Recordset_task, "text"));
...
while($row=mysql_fetch_assoc($query)){
  ...
  /*  if(!empty($t)){
    ...
    $dis[]=array("value"=>$row['summ1'],"name"=>$row['task_tpye']/*,"v"=>$v*/);
}

$len_dis=count($dis);
for($i=0;$i<$len_dis;$i++){
    $dis_name[]=$dis[$i]['name'];//提取作为图例
}
...
<!--          </tr>-->
    ...
    <div id="main_workhour" style="width:600px;height:230px"></div>
    <!-- ECharts单文件引入 -->
...
//                'echarts',
            ...
            function (ec) {
                ...
                var myChart = ec.init(document.getElementById('main_workhour'));
